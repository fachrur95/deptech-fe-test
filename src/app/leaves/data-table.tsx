...
export function DataTable<TData, TValue>({
  ...
  const { refetch, isFetching, data } = api.leave.findAll.useQuery(
    {
      limit: 10,
      page: currentPage,
      search,
  ...
  const mutationDelete = api.leave.destroy.useMutation({
    onSuccess: () => refetch(),
    onError: (error) => {
      toast({
        title: "Error message:",
  ...
  const handleDelete = (id: number) => {
    try {
      ...
      alert("Leave berhasil dihapus.");
    } catch (error) {
      console.error("Error deleting leave:", error);
      alert("Gagal menghapus leave.");
    }
  };
  ...
  useEffect(() => {
    if (data) {
      const dataResponse = data as unknown as PaginationResponse<ILeave>;
